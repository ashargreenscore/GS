...
// Migrate material categories to new structure
app.post('/api/migrate-categories', async (req, res) => {
  try {
    console.log('🔄 Category migration requested...');
    const result = await db.migrateMaterialCategories();
    res.json({ 
      success: true, 
      message: `Migration complete! Updated ${result.updated} out of ${result.total} materials`,
...
app.get('/api/admin/materials', async (req, res) => {
  try {
    ...
    if (listingType && listingType !== 'all') {
      materials = materials.filter(m => m.listing_type === listingType);
    }
